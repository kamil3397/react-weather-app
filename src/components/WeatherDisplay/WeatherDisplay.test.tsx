...
describe('WeatherDisplay', () => {
  ...
  it('displays weather information when weather is available', async () => {
    ...
    expect(await screen.findByRole('heading')).toHaveTextContent('Barcelona, Spain')

    expect(await screen.findByText('21')).toBeInTheDocument()
    expect(await screen.findByText('°C')).toBeInTheDocument()

    expect(await screen.findByText(mockWeather.current.condition.text)).toBeInTheDocument()

    const img = await screen.findByAltText(mockWeather.current.condition.text) as HTMLImageElement
    expect(img.src).toBe(`https:${mockWeather.current.condition.icon}`)

    expect(await screen.findByText(`${mockWeather.current.humidity}%`)).toBeInTheDocument()
    expect(await screen.findByText(`${mockWeather.current.wind_kph}Km/h`)).toBeInTheDocument()
  })
})